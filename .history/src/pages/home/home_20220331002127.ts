 ...
 removerAvaliados(animais: any[], avaliacoes: any[]){
  ...
  getAvaliados(){
    ...
    this.avaliacaoProvider.getAll().take(1).subscribe(item =>{
      this.dialogo.abreCarregando()
      this.avaliados = item;
      let novaLista = this.removerAvaliados(this.listaAnimais, this.avaliados);
      novaLista = novaLista.filter(x => x.brincadeira == 'brincalhao');
      ...
      if(!this.recomendaRandomico){
        ...
        listaRecomendados = this.recomendacao.cosineSimilaratyWeighed(this.adotante, novaLista).slice(0,5); 
      }else{
