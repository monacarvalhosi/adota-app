    ...
    contentData.find(function(element){
      if(element == item){
        for(let count = 0; count < weighedContent.length; count++){
            ...
            console.log('weighedContent[count].indexOf(element)', weighedContent[count][0].indexOf(element));
            if(weighedContent[count][0].indexOf(element) != -1 ){
                let numero = parseFloat(weighedContent[count][1]);
