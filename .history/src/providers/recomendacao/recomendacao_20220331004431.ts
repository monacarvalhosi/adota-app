...
animais:  Observable<any>;
  ...
  trataObjetoWeighed(item:any) {
    let vetor = [];
    vetor[0] = item.amigavel_crianca+item.relevancia_amigavel_crianca;
    vetor[1] = item.brincadeira+item.relevancia_brincadeira;
    vetor[2] = item.exercicio+item.relevancia_exercicio;
    vetor[3] = item.guarda+item.relevancia_guarda;
    vetor[4] = item.moradia+item.relevancia_moradia;
    vetor[5] = item.pelagem+item.relevancia_pelagem;
    vetor[6] = item.porte+item.relevancia_porte;
    vetor[7] = item.queda_pelo+item.relevancia_queda_pelo;
    vetor[8] = item.sexo+item.relevancia_sexo;
    vetor[9] = item.tendencia_latir+item.relevancia_tendencia_latir;
    console.log('vetor tendencia', vetor);
    return vetor
  }

  trataInteiros(item:any){
    console.log('trataInteiros', item);
    let vetor = [];
    for(let i = 0; i < item.length; i++){
      console.log('trataInteiros', !this.isNumber(item[i]));
